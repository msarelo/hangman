...
public class Dao {
    ...
    public List executeNamedQuery(String query, List<String> clazz, List<Object> o) {

        beginTransaction();
        TypedQuery queries = entityManager.createNamedQuery(persistedClass.getSimpleName() + "." + query, persistedClass);
        for (int i = 0; i < clazz.size(); i++) {
            queries.setParameter(clazz.get(i).toLowerCase(), o.get(i));
        }
        List result = queries.getResultList();
        commitAndCloseTransaction();
        return result;
